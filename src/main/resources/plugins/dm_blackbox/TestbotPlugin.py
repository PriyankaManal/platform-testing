    ...
    def runner(self, args, display=True):
        ...
        try:
            ...
            if req.status_code in [500, 404, 400, 409]:
                ...
                if title_tag:
                    error_msg = re.sub('<[A-Za-z\/][^>]*>', '', title_tag.group())
                else:
                    error_msg = req.text
            ...
            error_msg = 'Unable to connect to the Deployment Server'

        except RequestException:
            pass

        except ValueError:
            error_msg = 'In-valid JSON Response'

        except:
            error_msg = 'Unknown Exception - ' + sys.exc_info()[0]
        ...
        values.append(Event(TIMESTAMP_MILLIS(), 'deployment-manager',
                            'deployment-manager.health', [error_msg], health))
        if display:
